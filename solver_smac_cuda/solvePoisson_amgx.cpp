...
void setMatrixPoisson(solverConfig& cfg , mesh& msh , variables& v , matrix& mat_p )
{
    ...
    //temp v.copyVariables_cell_plane_D2H_all();

    // initialize
    for (auto& lhs_c : mat_p.lhs )
    {
        std::fill(lhs_c.begin(), lhs_c.end(), 0.0);
    }
    ...
    for (auto& bc : msh.bconds)
    {
        if (bc.valueTypes["P"] == 1)  // dirhicret
        {
            ...
            for (auto& ip : bc.iPlanes)
            {
                ...
                //printf("ic=%d, dn=%f, sx=%f, sy=%f, sz=%f, ss=%f\n", ic0, dn, sv[0], sv[1], sv[2], ss);
            }
        } 
    }
    ...
    for (geom_int ic=0 ; ic<msh.nCells; ic++)
    {
        ...
        //printf("ic=%d, divU=%f, ro=%f, dt=%f\n", ic, divU_vol[ic], ro[ic], cfg.dt);
    }
    //printf("18816 divU=%f, ro=%f, convx=%f, diffx=%f\n", divU_vol[18816], ro[18816], v.c["convx"][18816], v.c["diffx"][18816]);
...
void solvePoisson(solverConfig& cfg , mesh& msh , variables& var , matrix& mat_p )
{
    ...
    AMGX_SAFE_CALL(AMGX_vector_destroy(rh));
    AMGX_SAFE_CALL(AMGX_matrix_destroy(matrix));
    ...
    for (auto& bc : msh.bconds)
    {
        if (bc.valueTypes["P"] == 1)  // dirhicret
        {
            for (auto& ip : bc.iPlanes)
            {
                ...
                USp[ip] += -(0.0 - dP[ic0])/dn*ss*cfg.dt/ro[ic0];
            }
        } 
